...
WorldPacket* BuildQuestQueryResponse(Quest *qst)
{
    ...
    uint32 Quest_Money = sQuestMgr.GenerateQuestXP(0, qst);
    ...
    *data << uint32(qst->reward_xp_as_money);       // Copper given instead of XP
...
void QuestLogEntry::SaveToDB()
{
    ...
    sDatabase.Execute("DELETE FROM questlog WHERE player_guid=%u AND quest_id=%u", m_plr->GetGUIDLow(), m_quest->id);
    std::stringstream ss;
