...
//    ChatCommand *table = getCommandTable();
    ...
    if(!ShowHelpForCommand(m_session, getCommandTable(), cmd))
    {
...
bool ChatHandler::HandleStartCommand(const char* args, WorldSession *m_session)
{
    ...
    else if (m_plyr && args && strlen(args) > 2)
    {
        race = args;
        #ifdef WIN32
            // burlex: for some reason transform doesnt work on linux.. guess we'll have to find another way
            transform(race.begin(), race.end(), race.begin(), tolower);
        #else
            // Capt: lame fix for burlex his problem
            race = "";
...
bool ChatHandler::HandleGMListCommand(const char* args, WorldSession *m_session)
{
    ...
    for (itr = sObjHolder.Begin<Player>(); itr != sObjHolder.End<Player>(); itr++)
    {
        if(itr->second->GetSession() && itr->second->GetSession()->GetPermissionCount())
        {
            if(first)
