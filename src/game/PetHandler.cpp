...
void WorldSession::HandlePetAction(WorldPacket & recv_data)
{
    ...
    {
        Creature *pCharm = sObjHolder.GetObject<Creature>(petGuid);
        if(!pCharm) 
            return;
    ...
    Pet *pPet = sObjHolder.GetObject<Pet>(petGuid);
    if(!pPet)
        return;
    ...
    {
        recv_data >> targetguid;
        switch(UINT32_LOPART(GUID_HIPART(targetguid)))
        {
        case HIGHGUID_PLAYER:
            pTarget = sObjHolder.GetObject<Player>(targetguid);
            break;
        case HIGHGUID_UNIT:
            pTarget = sObjHolder.GetObject<Creature>(targetguid);
            break;
        case HIGHGUID_PET:
            pTarget = sObjHolder.GetObject<Pet>(targetguid);
            break;
        }
...
void WorldSession::HandleStablePet(WorldPacket & recv_data)
{
    ...
    pPet->Remove(false);    // no safedelete needed
}
