...
#define MAX_PASSIVE_AURAS 192   // burlex: i mananged to break this.. :p seems we need more

bool WOWD_SERVER_DECL Rand(float);
...
{                                          //   = 0x01
    ...
    U_FIELD_FLAG_UNIT_UNTACKABLE_SELECT        = 0x100000, //AI, UNIT can attack non players.
    //                                           = 0x200000,
    //                                           = 0x400000,
    //                                           = 0x800000,
...
//====================================================================
class WOWD_SERVER_DECL Unit : public Object
{
public:
    virtual ~Unit ( );
  ...
  //  void StrikeWithAbility(Unit *pVictim,Spell*spell,uint32 addspelldmg,uint32 damage_type);
   ...
   // void DealDamage(Unit *pVictim, uint32 damage, uint32 targetEvent, uint32 unitEvent, uint32 spellId = 0);   // to stop from falling, etc
    ...
    void Strike(Unit *pVictim,uint32 damage_type,SpellEntry *ability,int32 add_damage,int32 pct_dmg_mod,uint32);
//    void PeriodicAuraLog(Unit *pVictim, SpellEntry* spellID, uint32 damage, uint32 damageType);
    ...
    inline uint32 GetCP()
    {
     return ((GetUInt32Value(PLAYER_FIELD_BYTES) & 0xFF00) >> 8);
    
    }
    inline void SetCP(uint32 cp)
    {
         SetUInt32Value(PLAYER_FIELD_BYTES,((GetUInt32Value(PLAYER_FIELD_BYTES) & 0xFFFF00FF) | (cp << 8)));
    }
    ...
    //SM
    float * SM_CriticalChance;//flat
    int32 * SM_FDur;//flat
    int32 * SM_PDur;//pct
    float * SM_PRadius;//pct
    float * SM_FRadius;//flat
    float * SM_PRange;//pct
    float * SM_FRange;//flat
    ...
    float * SM_FResist;//flat
    int32 * SM_PAPBonus;//pct
    int32 * SM_PCost;
    int32 * SM_FCost;
    float * SM_PNonInterrupt;
    ...
    //SM
    //types of modifiers,once it's done will be handled in auras 
    //Phan
    ...
    int32 HealTakenPctMod[7];
    uint32 SchoolImmunityList[7];
    int32 SpellCritChanceSchool[7];
    int32 PowerCostMod[7];
    int32 CreatureAttackPowerMod[11];
    int32 CreatureRangedAttackPowerMod[11];
    ...
    int32 m_meleeDamageSplitPCT;
    int32 m_spellDamageSplitPCT;
    ...
    {
        SetUInt32Value(UNIT_FIELD_BYTES_0, ((GetUInt32Value(UNIT_FIELD_BYTES_0) & 0x00FFFFFF) | (type << 24)));
    }
