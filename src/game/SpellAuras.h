...
enum MOD_TYPES
{
    ...
    SPELL_AIRA_INCREASE_RAGE_FROM_DAMAGE_DEALT_PCT = 213,
    TOTAL_SPELL_AURAS = 214,
};
enum AuraTickFlags
{
...
class WOWD_SERVER_DECL Aura : public EventableObject
{
public:
    ...
    inline bool IsPassive(){return     (m_spellProto->Attributes & 64 && !m_areaAura);}
    ...
    void SpellAuraModCreatureRangedAttackPower(bool appy);
    ...
    void SpellAuraBreathing(bool apply);
    ...
    void SpellAuraIncreaseHpbyInt(bool apply);
    ...
    inline void SendPeriodicAuraLog(const uint64& CasterGuid, Unit * Target, uint32 SpellID, uint32 School, uint32 Amount, uint32 Flags)
    {
        WorldPacket data(SMSG_PERIODICAURALOG, 46);
        data << Target->GetNewGUID();           // target guid
