...
//====================================================================
class WOWD_SERVER_DECL Object : public EventableObject
{
public:
    ...
    //! Get uint32 property
    inline const uint32& GetUInt32Value( const uint16 &index ) const
    {
    ...
    //! Get uint64 property
    inline const uint64& GetUInt64Value( const uint16 &index ) const
    {
    ...
    //! Get float property
    inline const float& GetFloatValue( const uint16 &index ) const
    {
        ASSERT( index < m_valuesCount );
    ...
    inline void ModBaseUInt32Value( const uint32 index, const int32 value )
    {
        ASSERT( index < m_valuesCount );
        m_baseUint32Values[ index ] += value;
    }

    void __fastcall ModBaseFloatValue( const uint32 index, const float value )
    {
        ASSERT( index < m_valuesCount );
    ...
    inline const uint32& GetBaseUInt32Value( const uint32 index ) const
    {
        ASSERT( index < m_valuesCount );
        return m_baseUint32Values[ index ];
    }

    inline const uint64& GetBaseUInt64Value( const uint32 index ) const
    {
    ...
    //! Get float property
    const float& GetBaseFloatValue( const uint32 index ) const
    {
        ASSERT( index < m_valuesCount );
    ...
    //! Set uint32 property
    inline void SetBaseUInt32Value( const uint32 index, const uint32 &value )
    {
        ASSERT( index < m_valuesCount );
        m_baseUint32Values[ index ] = value;
    }
    ...
    //! Set uint64 property
    inline void SetBaseUInt64Value( const uint32 index, const uint64 &value )
    {
        ASSERT( index + 1 < m_valuesCount );
    ...
    //! Set float property
    inline void SetBaseFloatValue( const uint32 index, const float &value )
    {
        ASSERT( index < m_valuesCount );
        m_baseFloatValues[ index ] = value;
    }

    inline void SetBaseFlag( const uint32 index, uint32 newFlag )
    {
        ASSERT( index < m_valuesCount );
        m_baseUint32Values[ index ] |= newFlag;
    }

    inline void RemoveBaseFlag( const uint32 index, uint32 oldFlag )
    {
        ASSERT( index < m_valuesCount );
        m_baseUint32Values[ index ] &= ~oldFlag;
    }

    inline bool HasBaseFlag( const uint32 index, uint32 flag ) const
    {
        ASSERT( index < m_valuesCount );
    ...
    void SendMessageToGms(WorldPacket *data, bool self);
    void SendMessageToNonGms(WorldPacket *data, bool self);
	...
	void SpellNonMeleeDamageLog(Unit *pVictim, uint32 spellID, uint32 damage, bool allowProc);
    ...
    bool IsPet();
    inline void SetSpawnedObject(bool s) { m_SpawnedObject = s; }
    inline bool IsSpawnedObject() { return m_SpawnedObject; }

	//object faction
	void _setFaction();
    uint32 _getFaction(){return m_faction->myFaction;}
    ...
    void UpdateOfflinePlayerData();

protected:
    Object ( uint32 high = 0, uint32 low = 0 );
    ...
    // this will HALT the thread until its finished.
    // use for things like adding to world etc
    bool m_objectBusy;
    ...
    std::set<Object*> m_friendlyFactsInRange;
    std::set<Player*> m_inRangePlayers;
    
    //! GUID Storage
    //! NOT ALL OBJECTS WILL HAVE THOSE SO WATCH OUT
    uint32 m_GuidLow;
    uint32 m_GuidHigh;    
