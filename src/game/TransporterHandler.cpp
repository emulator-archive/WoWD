...
// Waypoint Generation code taken from Mangos, with modifications.
...
bool Transporter::CreateAsTransporter(uint32 EntryID, const char* Name, uint32 Time)
{
    ...
    GameObjectInfo *GoI = objmgr.GetGameObjectNameNULL(EntryID);
    if(!GoI) return false;

    Create(EntryID, HIGHGUID_TRANSPORTER, GoI->DisplayID, 1, EntryID, 1, GAMEOBJECT_TYPE_MO_TRANSPORT, 33, 40, 0,0,0,0, -0.486059f);
...
bool Transporter::GenerateWaypoints()
{
    ...
    //m_WayPoints[0] = pos;
    m_WayPoints.insert( WaypointMap::value_type( 0, pos) );
        ...
        if (((d < keyFrames[i + 1].distFromPrev) && (tTo > 0))) {
            while ((d < keyFrames[i + 1].distFromPrev) && (tTo > 0)) {
                ...
                if (d > 0) {
                    ...
                    /*if (teleport)*/       // Might want to reduce the waypoint interval some time
                                            // to cut down on cpu if needed.
                        //m_WayPoints[t] = pos;
...
void Transporter::TransportPassengers(uint32 mapid, uint32 oldmap, float x, float y, float z)
{
    ...
    if(mPassengers.size() > 0)
    {
        ...
        for(; itr != mPassengers.end();)
        {
            ...
            Player *plr = World::GetPlayer(it2->first);
            if(!plr)
            {
                // remove from map
...
void ObjectMgr::LoadTransporters()
{
    ...
    do 
    {
        ...
        Transporter * pTransporter = sObjHolder.Create<Transporter>(entry);
        if(!pTransporter->CreateAsTransporter(entry, "", period))
        {
            ...
            sObjHolder.Delete<Transporter>(pTransporter);
        }

        SetProgressBar(i, total, "Transporters");
        ++i;
