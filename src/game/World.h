...
class WOWD_SERVER_DECL World : public Singleton<World>, public EventableObject
{
public:
    World();
    ~World();
    ...
    WorldSession* FindSessionByName(std::string Name);
    ...
    inline uint32 GetNonGmSessionCount();
    ...
    static Unit*          GetUnit(const uint64 & guid);
    static Player*        GetPlayer(const uint64 & guid);
    static Creature*      GetCreature(const uint64 & guid);
    static GameObject*    GetGameObject(const uint64 & guid);
    static DynamicObject* GetDynamicObject(const uint64 & guid);

    uint32 mActiveCellCount;
    uint32 mInactiveCellCount;
    uint32 mHalfActiveCellCount;
    uint32 mTotalCells;
    
    uint32 mLoadedCreatures[2];
    uint32 mLoadedGameObjects[2];
    ...
    void EventDeleteInstance(uint32 mapid, uint32 instanceid);
