...
bool ChatHandler::HandleShowReactionCommand(const char* args, WorldSession *m_session)
{
    ...
    if (guid != 0)
    {
        obj = (Object*)sObjHolder.GetObject<Creature>(guid);
    }
...
bool ChatHandler::HandleMoveInfoCommand(const char* args, WorldSession *m_session)
{
    ...
    uint64 guid = m_session->GetPlayer()->GetSelection();
    if(!(obj = (Object*)sObjHolder.GetObject<Creature>(guid)))
    {
        SystemMessage(m_session, "You should select a character or a creature.");
        return true;
    }
...
bool ChatHandler::HandleSetBytesCommand(const char* args, WorldSession *m_session)
{
    ...
    obj->SetBaseUInt32Value(BytesIndex, ( ( Value1 ) | ( Value2 << 8 ) | ( Value3 << 16 ) | ( Value4 << 24 ) ) );
...
bool ChatHandler::HandleAggroRangeCommand(const char* args, WorldSession *m_session)
{
    ...
    if (guid != 0)
    {
        if(!(obj = sObjHolder.GetObject<Player>(guid)) && !(obj = sObjHolder.GetObject<Creature>(guid)))
        {
            SystemMessage(m_session,  "You should select a character or a creature.");
            return true;
        }
...
{
    Unit* target = NULL;
    target = sObjHolder.GetObject<Creature>(m_session->GetPlayer()->GetSelection());
    if(!target)
...
*/
    ...
    obj->SetBaseUInt32Value(field, value);
