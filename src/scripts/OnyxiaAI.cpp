...
public:
    ...
    void OnCombatStart(Unit* mTarget)
    {
        ...
        _unit->SendChatMessage(11, 0, "How fortuitous. Usually i have to leave my lair in order to feed.");
        if(m_useSpell)
...
{
    OnyxiaAI * ony = new OnyxiaAI(creature);
    if(!ony->HasEntry())
    {
        delete ony;
        return 0;
...
{
    // Onyxia
    sScriptMgr.ScriptAddCreatureModule(CN_ONYXIA, mod);
}
